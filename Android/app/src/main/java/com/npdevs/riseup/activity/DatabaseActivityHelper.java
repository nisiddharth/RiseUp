...
public class DatabaseActivityHelper extends SQLiteOpenHelper {
    ...
    public Cursor getLearningData() {
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor res = db.rawQuery("select * from " + TABLE_NAME + " WHERE TYPE='learning'", null);
        return res;
    }

    public Cursor getLeisureData() {
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor res = db.rawQuery("select * from " + TABLE_NAME + " WHERE TYPE='leisure'", null);
        return res;
    }

    public Cursor getProductiveData() {
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor res = db.rawQuery("select * from " + TABLE_NAME + " WHERE TYPE='productive'", null);
        return res;
    }
}